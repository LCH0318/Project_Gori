...
const Verification = () => {
    ...
    useEffect(() => {
        setIsAgreementAll(isAgreementPrivacy && isAgreementMarketing && isAgreementTerms);
        ...
        <>
            <div>
                ...
                <div className={`${styles['bottomSheet']} ${code.length === 5 ? styles["active"] : ""}`}
                    ref={sheetRef}
                    style={{
                        transform: code.length === 5 ? "TranslateY(0)" : `translateY(${bottomSheetHeight}px)`,
                        transition: "transform 0.3s case-in-out"
                    }}>
                    <div className={styles["useRef"]}>
                        <div>
                            <CheckBox setVal={agreementAllHandler} checked={isAgreementAll} text="약관 전체동의" />
                            {/* <label>
                        ...
                        <div>
                            <CheckBox setVal={setIsAgreementTerms} checked={isAgreementTerms} text="이용약관 동의(필수)" />
